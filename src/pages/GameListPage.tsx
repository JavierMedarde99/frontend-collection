  ...
  return (
    <section className="flex flex-col gap-24">
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4">
        <div>
          <h1 className="font-display text-heading-lg mb-2">Mis videojuegos</h1>
          <p className="text-body text-slate">
            {loading
              ? 'Cargando videojuegos…'
              : `${totalElements} videojuego${totalElements === 1 ? '' : 's'} en tu colección`}
          </p>
        </div>
        <div className="flex items-center gap-3 shrink-0">
          ...
          <Link className="btn-primary" to="/juegos/nuevo">
            <svg
              aria-hidden="true"
      ...
      <div className="flex flex-wrap items-center gap-2" role="group" aria-label="Filtrar por estado">
        ...
        {Object.entries(GAME_STATES).map(([key, label]) => (
          <FilterPill
            key={key}
            active={status === key}
            onClick={() => setQuery({ status: key as GameStatus })}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
            {games.map((game, index) => (
              <GameCard key={game.id} game={game} index={index}
                ...
                onDelete={async () => { await deleteGame(game.id); load() }}
              />
            ))}
          </div>
