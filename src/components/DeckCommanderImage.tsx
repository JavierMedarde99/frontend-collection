...
export default function DeckCommanderImage({ commanderName, size = 'md' }: DeckCommanderImageProps) {
  ...
  if (!imageUrl) {
    return (
      <div className={`${dims} rounded-xl shrink-0 bg-gradient-to-br from-brand-soft to-accent-soft border border-silver/60 flex items-center justify-center text-caption text-graphite text-center px-1`}>
        Sin imagen
      </div>
    )
  }
  ...
  return (
    <div className={`shrink-0 ${dims} overflow-hidden rounded-xl shadow-sm bg-paper`}>
      <img
        src={imageUrl}
        alt={commanderName}
        loading="lazy"
        className={`${dims} object-cover transition-transform duration-300 group-hover:scale-[1.05]`}
      />
    </div>
  )
}
