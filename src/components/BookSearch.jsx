            ...
            <article key={result.id} className="card flex gap-5">
              {result.coverImage ? (
                <img
                  ...
                  alt={result.title}
                  className="w-20 h-28 object-cover rounded shrink-0 bg-lifted-cream"
                />
              ) : (
                <div className="w-20 h-28 rounded shrink-0 bg-lifted-cream flex items-center justify-center text-eyebrow text-slate-gray">
                  Sin portada
                </div>
              )}
              <div className="min-w-0 flex-1">
                <h3 className="text-h3-card leading-snug mb-1">{result.title || 'Sin título'}</h3>
                <p className="text-body text-slate-gray">
                  {(result.authors && result.authors.join(', ')) || 'Autor desconocido'}
                </p>
                <p className="text-eyebrow text-slate-gray uppercase tracking-wide mt-1">
                  {(result.isbn ? `ISBN: ${result.isbn}` : '')}
                  {(result.publisher ? ` · ${result.publisher}` : '')}
                </p>
                {result.description && (
                  <p className="text-body text-slate-gray line-clamp-2 mt-2">
                    {result.description}
                  </p>
                )}
                <button
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink-black/30 backdrop-blur-sm">
          <div role="dialog" aria-modal="true" className="card w-full max-w-md">
            <h3 className="text-h3-card mb-1">{selected.title}</h3>
            <p className="text-body text-slate-gray mb-5">
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="label">
                  Tipo <span className="text-signal-orange">*</span>
                </label>
              ...
              <div>
                <label className="label">
                  Estado <span className="text-signal-orange">*</span>
                </label>
